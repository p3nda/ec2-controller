...
func (rm *resourceManager) sdkFind(
	...
	for _, elem := range resp.FlowLogs {
		...
		if elem.FlowLogId != nil {
			ko.Status.FlowLogID = elem.FlowLogId
		} else {
			ko.Status.FlowLogID = nil
		}
		...
		if elem.ResourceId != nil {
			ko.Status.ResourceID = elem.ResourceId
		} else {
			ko.Status.ResourceID = nil
		}
...
func (rm *resourceManager) sdkCreate(
	...
	if resp.FlowLogIds != nil {
		...
		ko.Status.FlowLogIDs = f1
	} else {
		ko.Status.FlowLogIDs = nil
	}
...
func (rm *resourceManager) newCreateRequestPayload(
	...
	if r.ko.Spec.ResourceIDs != nil {
		f9 := []*string{}
		for _, f9iter := range r.ko.Spec.ResourceIDs {
			var f9elem string
			f9elem = *f9iter
			f9 = append(f9, &f9elem)
		}
		res.SetResourceIds(f9)
	}
	if r.ko.Spec.ResourceType != nil {
		res.SetResourceType(*r.ko.Spec.ResourceType)
	}
	if r.ko.Spec.TagSpecifications != nil {
		f11 := []*svcsdk.TagSpecification{}
		for _, f11iter := range r.ko.Spec.TagSpecifications {
			f11elem := &svcsdk.TagSpecification{}
			if f11iter.ResourceType != nil {
				f11elem.SetResourceType(*f11iter.ResourceType)
			}
			if f11iter.Tags != nil {
				f11elemf1 := []*svcsdk.Tag{}
				for _, f11elemf1iter := range f11iter.Tags {
					f11elemf1elem := &svcsdk.Tag{}
					if f11elemf1iter.Key != nil {
						f11elemf1elem.SetKey(*f11elemf1iter.Key)
					}
					if f11elemf1iter.Value != nil {
						f11elemf1elem.SetValue(*f11elemf1iter.Value)
					}
					f11elemf1 = append(f11elemf1, f11elemf1elem)
				}
				f11elem.SetTags(f11elemf1)
			}
			f11 = append(f11, f11elem)
		}
		res.SetTagSpecifications(f11)
	}
